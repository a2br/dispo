  ...
  if (viewer?.id === inviter.id) {
    return (
      <main className="mx-auto max-w-md py-10 space-y-6">
        ...
        <ShareLinkButton url={`${appUrl()}/i/${code}`} title="Join me on dispo" text="See when we’re both free between classes:" label="Share my link" size="md" />
        <Link href="/" className="link text-sm">Back to dispo</Link>
      </main>
    );
  }
