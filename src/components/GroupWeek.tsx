        ...
        <div className="grid shrink-0 min-w-fit" style={{ gridTemplateColumns: cols }}>
          ...
          {members.map((m) => (
            <div key={m.id} className="border-b border-l border-line px-1 py-1.5 flex flex-col items-center gap-0.5 min-w-0">
              {m.isSelf ? <span className="size-5 rounded-sm bg-foreground text-background grid place-items-center text-[8px] font-bold">You</span> : <Avatar name={m.name} image={m.image} size={20} />}
