 ...
 */
function Tile({ name, size, firstOnly }: { name: string | null; size: number; firstOnly?: boolean }) {
...
export function shareCard({ eyebrow, title, subtitle }: { eyebrow?: string; title: string; subtitle: string }) {
  return frame(
    [wordmark, <Copy key="c" eyebrow={eyebrow} title={title} subtitle={subtitle} />, footer("EPFL timetables, side by side")],
 ...
 */
export function peopleCard({ eyebrow, title, subtitle, people }: { eyebrow?: string; title: string; subtitle: string; people: string[] }) {
  ...
  return frame(
    ...
    <div style={{ display: "flex", flex: 1, flexWrap: "wrap", alignItems: "center", alignContent: "center", justifyContent: "center", gap: 20 }}>
      ...
      {seat("you", <Tile name={null} size={size} />)}
    </div>,
  );
}
 ...
 */
export function weekCard({ name, weekStart, blocks }: { name: string; weekStart: number; blocks: Block[] }) {
  ...
  return frame(
    [
      ...
      <div key="c" style={{ display: "flex", flexDirection: "column", gap: 28 }}>
        ...
        <Copy eyebrow={fmtWeekLabel(weekStart)} title={`${first}’s week`} subtitle={`${fmtHours(free)} free between ${hh(minH)} and ${hh(maxH)}, Monday to Friday.`} />
      </div>,
      <div key="l" style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 22, color: MUTED }}>
        ...
        <span style={{ marginRight: 18 }}>Free</span>
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
      <div style={{ display: "flex", paddingLeft: 44, height: 44 }}>
        {byDay.map((_, d) => {
          ...
          return (
            <div key={d} style={{ display: "flex", flex: 1, justifyContent: "center", gap: 6, fontSize: 20, color: MUTED }}>
              {fmtDayShort(day)}
